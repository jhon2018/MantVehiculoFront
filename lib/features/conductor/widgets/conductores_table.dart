  ...
  Future<void> fetchConductores() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && decoded is Map<String, dynamic>) {
        var lista = List<Map<String, dynamic>>.from(decoded['conductor'] ?? const []);
        if (buscarGlobal) {
          final term = widget.searchTerm.toLowerCase();
          lista = lista.where((c) =>
              (c['nombre_completo'] ?? '').toString().toLowerCase().contains(term) ||
              (c['licencia'] ?? '').toString().toLowerCase().contains(term) ||
              (c['telefono'] ?? '').toString().toLowerCase().contains(term)).toList();
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Color(0xFF2B2626)),
            ),
          ],
        ),
      );
      ...
      return Padding(
        ...
        child: Column(
          children: [
            ...
            ElevatedButton(onPressed: fetchConductores, child: const Text('Reintentar')),
          ],
        ),
      );
    }
                ...
                children: [
                  ...
                  SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      rows: conductores.map<DataRow>((c) {
                        final id = (c['id_Conductor'] ?? '').toString();
                        final nombre = (c['nombre_completo'] ?? '').toString();
                        final dni = (c['dni'] ?? '').toString();
                        final licencia = (c['licencia'] ?? '').toString();
                        final cargo = (c['cargo'] ?? '').toString();
                            ...
                            DataCell(Row(
                              children: [
                                IconButton(
                                  ...
                                  onPressed: () {
                                    // Aquí abres modal y pasas el resto de datos si quieres
                                  },
                                ),
                                IconButton(
                  ...
                  SingleChildScrollView(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text('Página $currentPage • Registros: $totalRegistros',
                        ...
                        OutlinedButton(
                          ...
                          onPressed: (currentPage * pageSize) < totalRegistros ? _nextPage : null,
